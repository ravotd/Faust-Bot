...
class GlossaryModule(PrivMsgObserverPrototype):
    ...
    def update_on_priv_msg(self, data, connection: Connection):
        msg = data['message']
        if not -1 == msg.find(GlossaryModule._QUERY_EXPLANATION):
            self._answer_query(data, connection)
        elif not -1 == msg.find(GlossaryModule._REMOVE_EXPLANATION):
    ...
    def _answer_query(self, data, connection: Connection):
        ...
        if not len(split) == 2:
            return
        answer = glossary_provider.get_explanation(split[1])
